...
\posterbox[titlebox]{name=titlebox, below=top, column=1, span=12}{
  ...
  \large
    Impactos da IA na produção de código e a evolução da percepção sobre a IA.
}

\posterbox[footerbox]{name=footerbox, above=bottom, column=1, span=12}{
    \large
    Cássio Azevedo Cancio\par
    \vspace{4pt}
    \small\ttfamily
    cassio.acancio@usp.br\par
    \vspace{4pt}
    \footnotesize\rmfamily
    \textcolor{imesoftblue!30!white}
      {Departamento de Ciência da Computação --- Instituto de Matemática e Estatística}\relax
    \footimage{\includegraphics[width=10cm]{ime-logo}}
}
...
\posterbox[adjusted title = Contexto e Motivação]
    ...
    [cite start]Segundo o Gartner, a previsão é que \textbf{90\% dos engenheiros de software} utilizem assistentes de IA até 2028[cite: 1556].
